...
def extract_braced_command(
    ...
    """Extract \\command{...} arguments, supporting nested braces."""
    text = str(text)
    results: list[str] = []
...
def extract_math500_answer(
    ...
    """
    Same extraction policy as the third implementation:

    1. <answer>...</answer>
    2. \\boxed{...}
    3. The last occurrence of either Final Answer or Answer
    """
    text = str(text).strip()
    ...
    # 2. \boxed{...}
    boxed_answers = extract_braced_command(
        text,
        "boxed",
    )
    ...
    # 3. Take the last Final Answer / Answer marker.
...
def extract_math500_gold(
    ...
    """
    Same gold-extraction policy as the third implementation:

    <answer>...</answer> -> \\boxed{...} -> original text
    """
    if answer is None:
        return ""
...
def math500_reward_func(
    ...
    **_: Any,
    ...
    """
    Math500 reward using the third implementation's extraction policy.

    No extra answer normalization is performed.
    """
    prediction_text, prediction_method = (
        extract_math500_answer(completion)
    )

    target_text = extract_math500_gold(answer)
    ...
    try:
        ...
        print(
            "[math500_reward_func] "
            f"prediction_raw={prediction_text!r}, "
            f"target_raw={target_text!r}, "
...
def main():
    ...
    stem = f"{args.model_name}_{step_name}_{args.direction_type}_reward_landscape"
    ...
    def plot_landscape(std_column: str, filename: str, title_suffix: str) -> None:
        plt.figure(figsize=(8, 6))

        for direction_idx, subset in df.groupby("direction"):
            subset = subset.sort_values("coefficient")
            x = subset["coefficient"].to_numpy()
            mean_reward = subset["mean_reward"].to_numpy()
            uncertainty = subset[std_column].to_numpy()
            line = plt.plot(
                x, mean_reward, marker="o", linewidth=2,
                label=f"Direction {direction_idx}",
            )[0]
            plt.fill_between(
                x,
                np.clip(mean_reward - uncertainty, 0.0, 1.0),
                np.clip(mean_reward + uncertainty, 0.0, 1.0),
                alpha=0.2,
        ...
        plt.ylabel("Mean Math500 Reward")
        plt.title(f"Math500 Reward Landscape ({title_suffix})")
        plt.minorticks_on()
